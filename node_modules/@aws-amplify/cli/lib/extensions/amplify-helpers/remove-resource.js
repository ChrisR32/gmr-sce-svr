...
async function removeResource(context, category, resourceName, questionOptions = {}) {
    ...
    const confirm = (context.input.options && context.input.options.yes) ||
        (await context.amplify.confirmPrompt.run('Are you sure you want to delete the resource? This action deletes all files related to this resource from the backend directory.'));
    if (!confirm)
        return;
    try {
