...
async function createWalkthrough(context, templateParameters) {
    ...
    if (await context.amplify.confirmPrompt.run('Do you want to access other resources in this project from your Lambda function?')) {
        templateParameters = funcParamsUtils_1.merge(templateParameters, await execPermissionsWalkthrough_1.askExecRolePermissionsQuestions(context, templateParameters.functionName));
    }
...
async function updateWalkthrough(context, lambdaToUpdate) {
    ...
    if (await context.amplify.confirmPrompt.run('Do you want to update the Lambda function permissions to access other resources in this project?')) {
        funcParamsUtils_1.merge(functionParameters, await execPermissionsWalkthrough_1.askExecRolePermissionsQuestions(context, lambdaToUpdate, currentParameters.permissions));
